...
* H7ArcaneAcademy
* =============================================================================
*  Grants for a price a magic skill
* =============================================================================
*  Copyright 2015 Limbic Entertainment All Rights Reserved.
* ============================================================================= */

class H7ArcaneAcademy extends H7NeutralSite
...
function OnVisit( out H7AdventureHero hero )
{
	...
	if( hero.GetPlayer().IsControlledByAI() && class'H7AdventureController'.static.GetInstance().GetAI().IsAiEnabled())
	{
		...
		if( FulfillRequirements( hero ) && hero.GetPlayer().GetResourceSet().CanSpendResources( mCosts ) )
		{
			...
			hero.GetSkillManager().IncreaseRandomMagicSkillRank();
		}
	}
	else if( hero.GetPlayer().IsControlledByLocalPlayer() )
	{
		...
		{
			popUpMessage = class'H7Loca'.static.LocalizeSave("PU_ARCANE_ACADEMY","H7PopUp");
			class'H7PlayerController'.static.GetPlayerController().GetHud().GetRequestPopupCntl().GetRequestPopup().YesNoCostPopup( popUpMessage, class'H7Loca'.static.LocalizeSave("PU_ACCEPT","H7PopUp"), class'H7Loca'.static.LocalizeSave("PU_ABORT","H7PopUp"), Accept, none, mCosts );
		}
		else
		{
			if(CanNotImproveAnyMagicSkill(hero))
			{
				popUpMessage = class'H7Loca'.static.LocalizeSave("PU_MAGIC_DONT_FULFILL_REQ","H7PopUp");
				class'H7PlayerController'.static.GetPlayerController().GetHud().GetRequestPopupCntl().GetRequestPopup().OKPopup( popUpMessage, class'H7Loca'.static.LocalizeSave("PU_ABORT","H7PopUp") );
			}
			else
			{
...
function bool FulfillRequirements(H7AdventureHero hero)
{
	...
	ForEach skills(skill)
	{
		if(hero.GetSkillManager().CanIncreaseSkillRank(skill.GetID()) && !skill.IsMight() 
		   && skill.GetSchool() != ABILITY_SCHOOL_NONE 
		   && skill.GetSchool() != ALL
...
function bool CanNotImproveAnyMagicSkill(H7AdventureHero hero)
{
	local H7Skill skill;
	local array<H7Skill> skills;
	...
	ForEach skills(skill)
	{
		if(skill.IsMight() || skill.GetCurrentSkillRank() == SR_UNSKILLED) continue;
		if(hero.GetSkillManager().CanIncreaseSkillRank(skill.GetID()))
			return false;
	}
...
function H7TooltipData GetTooltipData(optional bool extendedVersion)
{
	...
	local bool hasUpgradeableMagicSkills;

	hero = class'H7AdventureController'.static.GetInstance().GetSelectedArmy().GetHero();
	
	hasUpgradeableMagicSkills = FulfillRequirements(hero);

	message = class'H7Loca'.static.LocalizeSave("TT_ARCANE_ACADEMY","H7AdvMapObjectToolTip");
	
	if(hero != none && HasVisited(hero))
	{
		visited = "<font color='#999999'>" $ class'H7Loca'.static.LocalizeSave("TT_VISITED","H7PermanentBonusSite") $ "</font>";
	}
	else if( hasUpgradeableMagicSkills )
	{
		visited = "<font color='#00ff00'>" $ class'H7Loca'.static.LocalizeSave("TT_CAN_IMPROVE","H7AdvMapObjectToolTip") $ "</font>";
	}
	else if(hero != none && CanNotImproveAnyMagicSkill(hero))
	{
