...
//=============================================================================
class H7AiActionUseSite_Exercise extends H7AiActionBase;
...
//	`LOG_AI("Action.UseSite_Exercise");
	...
	if(hero!=None)
	{
		...
		{
			case HRL_GENERAL:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteExercise.General; break;
			case HRL_MAIN:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteExercise.Main; break;
			case HRL_SECONDARY: actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteExercise.Secondary; break;
			case HRL_SCOUT:     actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteExercise.Scout; break;
			case HRL_SUPPORT:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteExercise.Support; break;
			case HRL_MULE:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteExercise.Mule; break;
		}
	...
	tensionValue = hero.GetAdventureArmy().GetAiTensionValue(AID_UseSiteExercise);
	...
	numSites=sic.GetBattleSiteNum();
//	`LOG_AI("Num Battle Sites:" @ numSites );
	if(numSites>actionCfg.ProximityTargetLimit) numSites=actionCfg.ProximityTargetLimit;
	for( k = 0; k < numSites; k++ )
	{
//		`LOG_AI("  Site" @ sic.GetBattleSite(k) );
		score.score =0.0f;
		score.dbgString = "Action.UseSite_Exercise; " $ sic.GetBattleSite(k) $ "; ";

		sic.SetTargetVisSite(sic.GetBattleSite(k)); // that sets internally the targetArmy to
		sic.SetTargetCellAdv(sic.GetBattleSite(k).GetEntranceCell());
...
//		`LOG_AI("  Available" @ ((utSiteAvailable[0] > 0.0f) ? "true" : "false") );
		if( utSiteAvailable.Length >= 1 && utSiteAvailable[0] > 0.0f )
		{
			...
			if( utAttackTarget.Length >= 1 && utAttackTarget[0] > 0.0f )
			{
				...
				score.score = utAttackTarget[0] * heroCfg.UseSiteExercise * aocMod;
			}

			score.dbgString = score.dbgString $ "heroCfg.UseSiteExercise(" $ heroCfg.UseSiteExercise $ ") aocMod(" $ aocMod $ ") ";
			...
			if( score.score > actionCfg.Cutoff )
			{
				...
				// todo: comment out when we can handle the battle mechanics (and have the expected outcome mangled into the utility score)
