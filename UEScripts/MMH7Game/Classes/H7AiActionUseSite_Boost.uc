...
//=============================================================================
class H7AiActionUseSite_Boost extends H7AiActionBase;
...
//	`LOG_AI("Action.UseSite_Boost");
	...
	if(hero!=None)
	{
		...
		{
			case HRL_GENERAL:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteBoost.General; break;
			case HRL_MAIN:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteBoost.Main; break;
			case HRL_SECONDARY: actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteBoost.Secondary; break;
			case HRL_SCOUT:     actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteBoost.Scout; break;
			case HRL_SUPPORT:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteBoost.Support; break;
			case HRL_MULE:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteBoost.Mule; break;
		}
	...
	tensionValue = hero.GetAdventureArmy().GetAiTensionValue(AID_UseSiteBoost);
...
//		`LOG_AI("  Site" @ sic.GetBuffSite(k) );
		...
		score.score =0.0f;
		score.dbgString = "Action.UseSite_Boost; " $ sic.GetBuffSite(k) $ "; ";
...
//		`LOG_AI("  Available" @ ((utSiteAvailable[0] > 0.0f) ? "true" : "false") );
		if( utSiteAvailable.Length >= 1 && utSiteAvailable[0] > 0.0f )
		{
			...
			if( utAttackTarget.Length >= 1 && utAttackTarget[0] > 0.0f )
			{
				...
				score.score = utAttackTarget[0] * heroCfg.UseSiteBoost * aocMod;
			}

			score.dbgString = score.dbgString $ "heroCfg.UseSiteBoost(" $ heroCfg.UseSiteBoost $ ") aocMod(" $ aocMod $ ") ";
