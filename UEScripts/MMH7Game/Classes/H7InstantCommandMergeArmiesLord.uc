...
//=============================================================================
class H7InstantCommandMergeArmiesLord extends H7InstantCommandBase;
...
function MPInstantCommand CreateMPCommand()
{
	...
	command.Type = ICT_MERGE_ARMIES_LORD;
...
function Execute()
{
	...
	success = mSite.MergeArmiesComplete( mFromArmy, mToArmy );
	...
	if(mSite.GetPlayer().IsControlledByLocalPlayer())
	{
		...
		{
			hud.GetTownHudCntl().CompleteMergeArmies(success);
		}
	}
}
