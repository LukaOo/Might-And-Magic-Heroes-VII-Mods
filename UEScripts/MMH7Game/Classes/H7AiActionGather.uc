...
//=============================================================================
class H7AiActionGather extends H7AiActionBase;
...
//	`LOG_AI("Action.Gather");
	...
	if(hero!=None)
	{
		...
		{
			case HRL_GENERAL:   actionCfg = cfg.mAiAdvMapConfig.mConfigGather.General; break;
			case HRL_MAIN:      actionCfg = cfg.mAiAdvMapConfig.mConfigGather.Main; break;
			case HRL_SECONDARY: actionCfg = cfg.mAiAdvMapConfig.mConfigGather.Secondary; break;
			case HRL_SCOUT:     actionCfg = cfg.mAiAdvMapConfig.mConfigGather.Scout; break;
			case HRL_SUPPORT:   actionCfg = cfg.mAiAdvMapConfig.mConfigGather.Support; break;
			case HRL_MULE:      actionCfg = cfg.mAiAdvMapConfig.mConfigGather.Mule; break;
		}
	...
	tensionValue = hero.GetAdventureArmy().GetAiTensionValue(AID_Gather);
...
//	`LOG_AI("Num Sites:" @ numSites );
	...
	{
		if( !sic.GetCollectibles(k).IsA('H7ItemPile') || CheckIfSiteIsLocked( sic.GetCollectibles(k), hero ) ) continue;

//		`LOG_AI("  Site" @ sic.GetCollectibles(k) @ sic.GetCollectibles(k).GetName() );
		score.score =0.0f;
		score.dbgString = "Action.Gather; " $  sic.GetCollectibles(k) $ "; ";
		...
		if(utTargetScore.Length>0)
		{
			...
			score.score = utTargetScore[0] * heroCfg.Gather * aocMod;
		}

		score.dbgString = score.dbgString $ "heroCfg.Gather(" $ heroCfg.Gather $ ") aocMod(" $ aocMod $ ") ";
...
function bool PerformAction( H7Unit unit, AiActionScore score )
{
	...
	if( unit != None && ctrl != None )
	{
		...
		if( army != None )
		{
			success = gridManager.DoAttackArmy( army.Location,  true, true );
			if( ( class'H7AdventureController'.static.GetInstance() != none && class'H7AdventureController'.static.GetInstance().GetConfig().mAiAdvMapConfig.mConfigOutputToLog ) || class'H7AdventureController'.static.GetInstance() == none ) ;
			return success;
		}
		if( site != None )
...
function bool CheckIfSiteIsLocked( H7VisitableSite site, H7AdventureHero evaluationHero )
{
	...
	foreach heroes( hero )
	{
		...
		{
			if( hero.GetAiLastScoreAction().GetAdvActionID() == AID_Gather && hero.GetAiLastScoreParam().GetVisSite( APID_2 ) == site )
			{
				return true;
			}
		}
