...
//=============================================================================
class H7AiActionUseSite_Study extends H7AiActionBase;
...
//	`LOG_AI("Action.UseSite_Study");
	...
	if(hero!=None)
	{
		...
		{
			case HRL_GENERAL:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteStudy.General; break;
			case HRL_MAIN:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteStudy.Main; break;
			case HRL_SECONDARY: actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteStudy.Secondary; break;
			case HRL_SCOUT:     actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteStudy.Scout; break;
			case HRL_SUPPORT:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteStudy.Support; break;
			case HRL_MULE:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteStudy.Mule; break;
		}
	...
	tensionValue = hero.GetAdventureArmy().GetAiTensionValue(AID_UseSiteStudy);
	...
	numSites=sic.GetStudySiteNum();
//	`LOG_AI("Num Study Sites:" @ numSites );
	if(numSites>actionCfg.ProximityTargetLimit) numSites=actionCfg.ProximityTargetLimit;
	for( k = 0; k < numSites; k++ )
	{
//		`LOG_AI("  Site" @ sic.GetStudySite(k) );
		score.score =0.0f;
		score.dbgString = "Action.UseSite_Study; " $ sic.GetStudySite(k) $ "; ";

		sic.SetTargetVisSite(sic.GetStudySite(k)); // that sets internally the targetArmy to
		sic.SetTargetCellAdv(sic.GetStudySite(k).GetEntranceCell());
...
//		`LOG_AI("  Available" @ ((utSiteAvailable[0] > 0.0f) ? "true" : "false") );
		if( utSiteAvailable.Length >= 1 && utSiteAvailable[0] > 0.0f )
		{
			...
			if( utAttackTarget.Length >= 1 && utAttackTarget[0] > 0.0f )
			{
				...
				score.score = utAttackTarget[0] * heroCfg.UseSiteStudy * aocMod;
			}

			score.dbgString = score.dbgString $ "heroCfg.UseSiteStudy(" $ heroCfg.UseSiteStudy $ ") aocMod(" $ aocMod $ ") ";
