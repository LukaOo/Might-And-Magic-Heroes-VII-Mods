...
//=============================================================================
class H7AiActionUseSite_Observe extends H7AiActionBase;
...
//	`LOG_AI("Action.UseSite_Observe");
	...
	if(hero!=None)
	{
		...
		{
			case HRL_GENERAL:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteObserve.General; break;
			case HRL_MAIN:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteObserve.Main; break;
			case HRL_SECONDARY: actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteObserve.Secondary; break;
			case HRL_SCOUT:     actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteObserve.Scout; break;
			case HRL_SUPPORT:   actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteObserve.Support; break;
			case HRL_MULE:      actionCfg = cfg.mAiAdvMapConfig.mConfigUseSiteObserve.Mule; break;
		}
	...
	tensionValue = hero.GetAdventureArmy().GetAiTensionValue(AID_UseSiteObserve);
	...
	numSites=sic.GetObservatorySiteNum();
//	`LOG_AI("Num Observatory Sites:" @ numSites );
	if(numSites>actionCfg.ProximityTargetLimit) numSites=actionCfg.ProximityTargetLimit;
	for( k = 0; k < numSites; k++ )
	{
//		`LOG_AI("  Site" @ sic.GetObservatorySite(k) );
		
		score.score =0.0f;
		score.dbgString = "Action.UseSite_Observe; " $ sic.GetObservatorySite(k) $ "; ";

		sic.SetTargetVisSite(sic.GetObservatorySite(k)); // that sets internally the targetArmy to
		sic.SetTargetCellAdv(sic.GetObservatorySite(k).GetEntranceCell());
...
//		`LOG_AI("  Available" @ ((utSiteAvailable[0] > 0.0f) ? "true" : "false") );
		if( utSiteAvailable.Length >= 1 && utSiteAvailable[0] > 0.0f )
		{
			...
			if( utAttackTarget.Length >= 1 && utAttackTarget[0] > 0.0f )
			{
				...
				score.score = utAttackTarget[0] * heroCfg.UseSiteObserve * aocMod;
			}

			score.dbgString = score.dbgString $ "heroCfg.UseSiteObserve(" $ heroCfg.UseSiteObserve $ ") aocMod(" $ aocMod $ ") ";
