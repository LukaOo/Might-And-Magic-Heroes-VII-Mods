...
//=============================================================================
class H7AiActionPlunder extends H7AiActionBase;
...
//	`LOG_AI("Action.Plunder");
	...
	if(hero!=None)
	{
		...
		{
			case HRL_GENERAL:   actionCfg = cfg.mAiAdvMapConfig.mConfigPlunder.General; break;
			case HRL_MAIN:      actionCfg = cfg.mAiAdvMapConfig.mConfigPlunder.Main; break;
			case HRL_SECONDARY: actionCfg = cfg.mAiAdvMapConfig.mConfigPlunder.Secondary; break;
			case HRL_SCOUT:     actionCfg = cfg.mAiAdvMapConfig.mConfigPlunder.Scout; break;
			case HRL_SUPPORT:   actionCfg = cfg.mAiAdvMapConfig.mConfigPlunder.Support; break;
			case HRL_MULE:      actionCfg = cfg.mAiAdvMapConfig.mConfigPlunder.Mule; break;
		}
	...
	tensionValue = hero.GetAdventureArmy().GetAiTensionValue(AID_Plunder);
	...
	// for all enemy aoc site targets (that may have an defending army) and are below the not explored fow ...
	numSites=sic.GetOppAoCSiteNum();
//	`LOG_AI("Num opponent aoc Sites:" @ numSites );
	if(numSites>actionCfg.ProximityTargetLimit) numSites=actionCfg.ProximityTargetLimit;
	for( k = 0; k < numSites; k++ )
	{
//		`LOG_AI("  Site" @ sic.GetOppAoCSite(k) );
		suppressSite=false;
		site=sic.GetOppAoCSite(k);
		if( CheckIfSiteIsLocked( site, hero ) ) { continue; }
		if( site.GetPlayer().GetPlayerNumber() != PN_NEUTRAL_PLAYER && 
			site.GetPlayer().GetPlayerNumber() != currentUnit.GetPlayer().GetPlayerNumber() &&
			site.IsA('H7Mine') == true && !H7Mine( site ).IsPlundered() )
		{
			...
			if(aocSiteVassal!=None)
			{
				...
				if(aocSiteLord!=None)
				{
					// not me but controlled by ally
					if( currentUnit.GetPlayer().IsPlayerAllied( aocSiteLord.GetPlayer() ) == true )
					{
						suppressSite=true;
					}
				}
			...
			score.score =0.0f;
			score.dbgString = "Action.Plunder; " $  sic.GetOppAoCSite(k) $ "; ";

			sic.SetTargetVisSite(sic.GetOppAoCSite(k)); // that sets internally the targetArmy to
	...
	//		`LOG_AI("  Available" @ ((utSiteAvailable[0] > 0.0f) ? "true" : "false") );
			if( utSiteAvailable[0] > 0.0f && suppressSite==false )
			{
				...
				if( utAttackTarget.Length >= 1 && utAttackTarget[0] > 0.0f )
				{
					...
					score.score = utAttackTarget[0] * heroCfg.Plunder * aocMod;
				}

				score.dbgString = score.dbgString $ "heroCfg.Plunder(" $ heroCfg.Plunder $ ") aocMod(" $ aocMod $ ") ";
...
function bool CheckIfSiteIsLocked( H7VisitableSite site, H7AdventureHero evaluationHero )
{
	...
	foreach heroes( hero )
	{
		...
		{
			if( hero.GetAiLastScoreAction().GetAdvActionID() == AID_Plunder && hero.GetAiLastScoreParam().GetVisSite( APID_2 ) == site )
			{
				return true;
			}
		}
