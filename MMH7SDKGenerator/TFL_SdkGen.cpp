...
std::vector<std::string> _statics_list;
...
void GenerateScriptStruct ( UScriptStruct* pScriptStruct )
{
	...
	// vars
	DWORD64 dwSize = 0;
	DWORD64 dwLastOffset = 0;
	DWORD64 dwMissedOffset = 0;
	...
	else
	{
		...
		// stream to main buffer
		ssStreamBuffer0 << "// " << SDKMC_SSHEX ( dwSize, 8 ) << "\n";
	...
	ssStreamBuffer0 << "//\t vPoperty_Size=" << vProperty.size() << "\n"; 

	// process struct properties
	if( false ) {}
    #ifdef STRUCT_PROPERTIES_IFPOINTER
	else if ( sSStructFullName ==  "ScriptStruct Core.Object.Pointer" )			// FPointer
	      ssStreamBuffer0 << STRUCT_PROPERTIES_FPOINTER;
	#endif
	else
	{
...
{
	static std::vector<DWORD64> _stack;
...
//	fprintf(pLog, "FullStructName: %s\n", sScriptStructFullName.c_str());
	...
	{	
		...
		// loop protected
		if( std::find( _stack.begin(), _stack.end(), (DWORD64)pScriptStruct ) != _stack.end() ) return;
		_stack.push_back((DWORD64)pScriptStruct);
...
//			fprintf(pLog,"\tDump script: %s (0x%p) with property %s (0x%p) type: %d\n", pScriptStruct->GetName(), pScriptStruct, pStructProperty->GetName(), pStructProperty, iTypeResult );
...
void GenerateVirtualFunc ( UClass* pClass )
{
	...
	// stream to main buffer
	ssStreamBuffer0 << "\n\t// Virtual Functions  (" << SDKMC_SSHEX(pClass->VfTableObject.Dummy, 2) << ")\n";

	DWORD64 VfTable = (DWORD64) ( pClass->VfTableObject.Dummy );

	for ( DWORD64 i = 0x0; i < 0x800; i += 0x8 )
	{
		// check if pointed function is the ProcessEvent
		//if ( TFLHACKT00LS::FindPattern( *(DWORD64*) ( VfTable + i ), 0x200, (unsigned char*) ProcessEvent_Pattern, (char*) ProcessEvent_Mask ) )
		if( i == ProcessEvent_Offset)
		{
			// stream to main buffer (virtual function ProcessEvent)
			ssStreamBuffer0 << "\tvirtual void ProcessEvent ( class UFunction* pFunction, void* pParms, void* pResult = NULL );\t\t\t\t// " << SDKMC_SSHEX ( ( *(DWORD64*) ( VfTable + i ) ), 8 ) << " (" << SDKMC_SSHEX ( i, 2 ) << ")\n";
		...
		else
		{
			...
			// ssStreamBuffer0 << "// " << SDKMC_SSHEX((VfTable + i),8) << "idx: " << i << "ProcessEventIdx: " << ProcessEvent_Index << "\n";
			ssStreamBuffer0 << "\tvirtual void VirtualFunction" << SDKMC_SSDEC ( ( i / 0x8 ), 2 ) << " ( );\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t// " << SDKMC_SSHEX ( ( *(DWORD64*) ( VfTable + i ) ), 8 ) << " (" << SDKMC_SSHEX ( i, 2 ) << ")\n";
		}
	}
...
void GenerateClass ( UClass* pClass )
{
	...
	ssStreamBuffer0 << "// " << sClassFullName << " ( Property size: " << vProperty.size()  << " iter: " << classProps <<") \n";
	ssStreamBuffer0 << "// Class name index: " << pClass->Name.Index << " \n";
	...
	else																	// normal classes
	{
		...
		if ( dwLastOffset < pClass->PropertySize )
		{
			...
			ssStreamBuffer0 << "//\t LastOffset: " << dwLastOffset << "\n";
			ssStreamBuffer0 << "//\t Class Propsize: " << pClass->PropertySize << "\n" ;
	...
	_statics_list.push_back(std::string("UClass* " + sClassNameCPP + "::pClassPointer = NULL;\n\n"));
...
void ProcessPackages()
{
	...
	fprintf(pLog, "Pointer in process: 0x%p  - Num: %d",  UObject::GObjObjects(), UObject::GObjObjects()->Num() );
	fprintf(pLog,  "Data: 0x%p, 0x%llX", UObject::GObjObjects()->Data, (DWORD64)UObject::GObjObjects()->Data );
	for ( int i = 0; i < UObject::GObjObjects()->Num(); i++ ){
		UObject* pObject = UObject::GObjObjects()->Data[ i ];
		if ( ! pObject )
			continue;
		fprintf(pLog, "Object: %s - idx: %d - nidx: %d - pointer: 0x%p - 0x%llX\n", pObject->GetFullName(), i, pObject->Name.Index, pObject, (DWORD64) pObject );
	}
...
void Init_Core()
{
	...
	DWORD64 baseAddress = (DWORD64)miGame.lpBaseOfDll;
	// get GObjects
	GObjects =  baseAddress + GObjects_Offset;  // *(unsigned long*) ( TFLHACKT00LS::FindPattern( (unsigned long) miGame.lpBaseOfDll, miGame.SizeOfImage, (unsigned char*) GObjects_Pattern, (char*) GObjects_Mask ) + GObjects_Offset );
	
	fprintf ( pLog, "Base address: 0x%p\n", miGame.lpBaseOfDll, baseAddress);

	// log
	fprintf ( pLog, "GObjects: 0x%llX\n", GObjects );

	#ifdef GNames_Pattern
	// get GNames
	GNames =  baseAddress + GNames_Offset; // *(unsigned long*) ( TFLHACKT00LS::FindPattern( (unsigned long) miGame.lpBaseOfDll, miGame.SizeOfImage, (unsigned char*) GNames_Pattern, (char*) GNames_Mask ) + GNames_Offset );
	
	// log
	fprintf ( pLog, "GNames:   0x%llX\n", GNames );
	#endif
...
void Final_SdkHeaders()
{
	...
	sprintf_s ( cBuffer, "%s\\%s\\SdkBase.h", SDK_BASE_DIR, GAME_NAME_S );
	fopen_s ( &pFile, cBuffer, "w+" );

	PrintFileHeder ( "SdkBase", "h" );
	
	PrintSectionHeader ( "Defines" );
	fprintf ( pFile, "// #define GObjects\t\t\t0x%0llX\n", GObjects );
	fprintf ( pFile, "extern DWORD64 GObjects;\n", GObjects );
	fprintf ( pFile, "#define GObjects_Offset\t\t\t0x000000000%0llXll\n", GObjects_Offset );
	#ifdef GNames_Pattern
	fprintf ( pFile, "// #define GNames\t\t\t\t0x%0llX\n", GNames );
	fprintf ( pFile, "externt DWORD64 GNames;\n", GNames );
	fprintf ( pFile, "#define GNames_Offset\t\t\t\t0x000000000%0llXll\n", GNames_Offset );
	#endif

	fprintf ( pFile, "#define ProcessEvent_Offset\t\t\t0x%0llX\n", ProcessEvent_Offset );
	fprintf ( pFile, "#define ProcessEvent_Index\t\t\t0x%0llX\n", ProcessEvent_Index );
	
	PrintSectionHeader ( "Structs" );
	fprintf ( pFile, BASIC_STRUCTS );
	fprintf ( pFile, "// End of Base structs \n\n" );
	...
	fprintf ( pFile, "#include \"SdkBase.h\"" );
	...
	PrintSectionHeader ( "Statics initializations" );

	std::vector<std::string>::iterator it = _statics_list.begin();
	for(  ; it != _statics_list.end(); ++it ) fprintf(pFile, it->c_str() );
	fclose ( pFile );
}
