...
// VMTBaseManager
NOINLINE VMTBaseManager& VMTBaseManager::Init( void* inst, unsigned int offset, unsigned int vfuncs )
{
...
// VMTBasePointer
NOINLINE VMTBasePointer& VMTBasePointer::Init( void* inst, CallGateFn gate, unsigned int vfuncs )
{
	_inst = inst;
	_gate = gate;
	void** vmt = *(void***) inst;
...
NOINLINE void VMTBasePointer::Kill()
{
	if ( _dummy.vtable )
	{
...
// VMTBaseHook
NOINLINE VMTBaseHook& VMTBaseHook::Init( void** vmt, unsigned int vfuncs )
{
	_vftable = vmt;
	if ( !vfuncs )
	{
...
NOINLINE void VMTBaseHook::Kill()
{
	if ( _backup )
	{
		EraseHooks();
...
{
	return WriteMemory( dest, src, bytes );
}
...
public:
	virtual ~VMTDummy() { }
	virtual bool Login( const char* name, const char* passwd )
	{
		// Some cheesy login thing
		if ( authed = !strcmp( name, "admin" ) && !strcmp( passwd, "1234" ) )
		{
			rndkey = rand();
		}
		return authed;
	}
	virtual bool IsAuthed() const
	{
		return authed;
	}
	virtual unsigned int GetKey()
	{
		return IsAuthed()? rndkey: 0;
	}
	virtual bool LaunchNukes( unsigned int key )
	{
		if ( IsAuthed() )
		{
			return true;
...
bool TestDummy( VMTDummy* d )
{
	if ( !d->Login( "admin", "hacked!" ) )
		return false;
	if ( !d->IsAuthed() )
		return false;
	if ( !d->LaunchNukes( d->GetKey() ) )
		return false;
	return true;
}
